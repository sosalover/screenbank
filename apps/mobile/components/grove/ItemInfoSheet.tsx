...
export function ItemInfoSheet({ build, onClose }: ItemInfoSheetProps) {
  ...
  const isInProgress = build.status === 'in_progress' || build.status === 'delayed';
    ...
    <View style={[styles.container, { paddingBottom: bottom + 16 }]}>
      ...
      <View style={styles.row}>
        <View style={[styles.iconWrap, isDelayed && styles.iconWrapDelayed]}>
          <Ionicons name={build.cause.icon as any} size={32} color={isDelayed ? '#ef4444' : '#16a34a'} />
        </View>
      ...
      <View style={styles.statusRow}>
        {isInProgress ? (
          <>
            <View style={[styles.badge, isDelayed ? styles.badgeDelayed : styles.badgeActive]}>
              <Ionicons
                name={isDelayed ? 'eye' : 'hammer-outline'}
                size={13}
                color={isDelayed ? '#ef4444' : '#16a34a'}
              />
              <Text style={[styles.badgeText, isDelayed ? styles.badgeTextDelayed : styles.badgeTextActive]}>
                {isDelayed ? 'Delayed by The Algorithm' : 'In Progress'}
              </Text>
            </View>
            <Text style={styles.timer}>{formatTimeRemaining(build.completesAt)} remaining</Text>
          </>
        ) : (
          <View style={[styles.badge, styles.badgeComplete]}>
            <Ionicons name="checkmark-circle" size={13} color="#16a34a" />
            <Text style={[styles.badgeText, styles.badgeTextActive]}>Complete</Text>
          </View>
        )}
      </View>
